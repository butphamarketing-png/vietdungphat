...
export async function ensureMediaBucket() {
  ...
  if (data) {
    await client.storage.updateBucket(name, { public: true, fileSizeLimit: limit });
    return name;
  }
...
export async function createSignedUpload({ key }) {
  const client = getAdminClient();
  if (!client) throw new Error("Supabase chưa cấu hình (thiếu env)");
  const bucket = await ensureMediaBucket();
  const { data, error } = await client.storage.from(bucket).createSignedUploadUrl(key, { upsert: true });
  if (error || !data?.signedUrl) throw new Error(`signed upload: ${error?.message || "không tạo được URL"}`);
  const { data: pub } = client.storage.from(bucket).getPublicUrl(key);
  const url = pub?.publicUrl;
  if (!url) throw new Error("Không lấy được public URL");
  return { signedUrl: data.signedUrl, token: data.token, path: data.path || key, key, url, bucket };
}
